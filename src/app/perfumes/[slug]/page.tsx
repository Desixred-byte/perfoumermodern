      ...
      <div className="mx-auto max-w-[1540px] px-6 md:px-10">
        <div className="grid gap-8 xl:grid-cols-[0.98fr_1fr] xl:gap-12 xl:items-start">
          ...
          <div className="space-y-6 xl:pt-2">
            ...
            <div className="overflow-hidden rounded-[1.9rem] bg-[linear-gradient(180deg,rgba(255,255,255,0.97)_0%,rgba(249,249,247,0.92)_100%)] shadow-[0_22px_54px_rgba(24,24,24,0.06)] ring-1 ring-zinc-200/80">
              <div className="flex items-center justify-between border-b border-zinc-200/70 px-5 py-4 md:px-6">
                <p className="text-[0.78rem] font-medium tracking-[0.24em] text-zinc-400 uppercase">
                  {t.detail.sizePrice}
                </p>
                <p className="text-sm text-zinc-400">{t.detail.choose}</p>
              </div>
              {perfume.sizes.length ? (
                perfume.sizes.map((size) => (
                  <div
                    key={size.label}
                    className="group flex items-center gap-4 border-b border-zinc-200/55 px-4 py-4 last:border-b-0 md:px-6"
                  >
                    <div className="flex h-15 w-15 shrink-0 items-center justify-center rounded-[1.1rem] bg-white shadow-[0_10px_22px_rgba(24,24,24,0.06)] ring-1 ring-zinc-200/80">
                      <Image
                        src={perfume.image}
                        alt={perfume.imageAlt || perfume.name}
                        width={50}
                    ...
                    <div className="min-w-0">
                      <p className="text-[1.16rem] tracking-[-0.02em] text-zinc-800 md:text-[1.24rem]">
                        <span className="font-medium">{size.ml}</span>
                        <span className="ml-1 text-zinc-500">ML</span>
                      </p>
                      <p className="mt-0.5 text-[0.8rem] tracking-[0.18em] text-zinc-400 uppercase">
                        {t.detail.premiumSize}
                      </p>
                    </div>
                    <div className="ml-auto text-right">
                      <p className="stat-value text-[2rem] leading-none tracking-[-0.04em] text-zinc-900 md:text-[2.2rem]">
                        {size.price}
                        <span className="ml-1 text-[0.78em] text-zinc-700">₼</span>
                      </p>
                      <p className="mt-1 text-[0.74rem] tracking-[0.16em] text-zinc-400 uppercase">
                        {t.detail.readyPrice}
                      </p>
                    </div>
                  </div>
        ...
        <section className="mt-20">
          ...
          <div className="mt-9 flex justify-center">
            <Link
              href="/catalog"
              className="inline-flex min-h-13 items-center justify-center rounded-full border border-zinc-400 bg-transparent px-9 text-lg font-medium text-zinc-700 transition hover:bg-white/70"
            >
